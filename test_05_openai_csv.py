...
def analyze_aspect_sentiment(review, aspect):
    ...
    try:
        response = client.chat.completions.create(
            ...
            # messages=[
            #     {"role": "system", "content": (
            #         "You are a sentiment analysis expert. Your task is to assess customer reviews for British Gas Insurance "
            #         "based on specific aspects, such as 'value for money' or 'timeliness'. For each review, provide a sentiment score "
...
def process_reviews(input_csv, output_csv, aspects):
    ...
    for index, row in df.iloc[processed_count:].iterrows():
        ...
        for aspect in aspects:
            try:
                score, prod, reason = analyze_aspect_sentiment(review, aspect)
            except ValueError as ve:
                # Handle unpacking or formatting errors
                score, prod, reason = 0, "Error", f"Error ValueError: {str(ve)}"
            except Exception as e:
                # Catch any other errors
                score, prod, reason = 0, "Error", f"Error Exception: {str(e)}"
        ...
        # Print progress
        print(
            f"Review {reviews_processed}/{len(df)} - Elapsed: {format_time(total_elapsed_time)} - Remaining: {format_time(estimated_time_remaining)}, Unknown {unknown_count}, {unknown_count / reviews_processed:.1%}")
...
input_file = "BG_Trustpilot_AI_Sentiment_Predictive_Analysis_Clean.csv" #"BG_20pct_Reviews_March_Onwards.csv"  # Replace with your input file path
output_file = "BG_LLM_Full_Output_Prod_Cat_07.csv"  # Replace with your output file path
